...
library(readr)
ar <- read_delim(
  "articulation_speed_by_country.csv",
  delim     = ";",
  locale    = locale(decimal_mark = ".")
) %>% clean_names()
sr <- read_delim(
  "speech_speed_by_country.csv",
  delim     = ";",
  locale    = locale(decimal_mark = ".")
) %>% clean_names()
...
# Ländercode für Hervorhebung (graue Farbe) - z.B. "ARG" oder NULL für keine Hervorhebung
highlighted_country <- c("TAGESSCHAU")
...
p_sp <- sp %>%
  ...
  geom_line(aes(group = country), colour = "grey70") +
  geom_point(size = 3) +
  labs(title = "Speech rate – lectura vs. libre",
       x = "Words per minute", y = NULL, colour = "Mode") +
  theme_minimal(base_size = 12)
